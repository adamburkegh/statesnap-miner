...
from pm.ssnap.ssnap import (StateSnapshot, sslogFromCSV, sslogToCSV, 
                            sslogWithRanges)
        ...
        sslog = \
            {1: [ StateSnapshot(1,1801,
             ...
             2: [ StateSnapshot(2,1801,
                  ...
                  StateSnapshot(2,1805,
                                set(['Bludger']) ) ] }
        cf = tempfile.NamedTemporaryFile(delete_on_close=False)
        sslogToCSV(sslog, cf.name, 'personid','job','year' )
        cf.close()
        sslog2 = sslogFromCSV(cf.name, 'personid','job','year',
                             types={'personid': int, 'year':int },
                             keepSuccDupes=False)
        self.assertEqual( sslog, sslog2 )
