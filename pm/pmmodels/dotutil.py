...
'''
Local copy and paste fork from koalas to troubleshoot. Can be removed when koalas is patched and released. Current is koalas v0.3.1
'''
...
def dot_to_img(dot:str,dotf:str,imgf:str,outformat:str,dfenc:str=DFENC):
    with open(dotf,'w',encoding=dfenc) as outf:
        outf.write(dot)
        outf.write('\n')
    tfi = tempfile.NamedTemporaryFile(mode='w+t',delete=False)
    tfi.close()
    cmdout = subprocess.run(['dot','-T' + outformat,'-o' + tfi.name, dotf])
    shutil.copy(tfi.name,imgf)
    os.remove(tfi.name)
